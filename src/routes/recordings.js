...
recordingsRouter.get("/recordings/:recordingSid.mp3", async (req, res) => {
  ...
  try {
    ...
    // Twilio returns audio/mpeg for .mp3.
    // IMPORTANT: do not buffer the entire recording into memory (can be large and slow).
    // Stream it to the client to avoid 30s timeouts and "endless loading" symptoms.
    res.status(200);
    ...
    // Convert WHATWG ReadableStream -> Node.js Readable and pipe.
    const nodeStream = Readable.fromWeb(r.body);
    nodeStream.on("error", (e) => {
      logger.warn("Recording proxy stream error", { recordingSid, error: e?.message || String(e) });
      // If headers weren't sent yet, send a deterministic error.
      if (!res.headersSent) res.status(502).send("twilio stream error");
      else res.end();
    });
