...
function publicRecordingUrl(recordingSid) {
  ...
  // Keep the public URL stable (singular) and let the server support both.
...
async function proxyRecordingMp3(recordingSid, res, logger) {
  ...
  try {
    ...
    // Twilio may redirect to a CDN URL (signed). If so, follow manually so we don't lose auth.
    ...
    const isRedirect = [301, 302, 303, 307, 308].includes(first.status);
    const location = first.headers.get('location');
    const resp = isRedirect && location
